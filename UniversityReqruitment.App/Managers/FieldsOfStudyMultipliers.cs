...
namespace UniversityRecruitment.App.Managers
{
    public static class FieldsOfStudyMultipliers
    {
        public static Dictionary<string, float> GetMultipliers(Helpers.FieldsOfStudy fieldsOfStudy)
        {
            ...
            {
                multipliers = CreateMulipliers(0f, 0.8f, 2.0f, 0.2f, 0.4f, 1.6f);
            }
            else if (fieldsOfStudy == Helpers.FieldsOfStudy.Electrotechnics)
            {
                multipliers = CreateMulipliers(0f, 1.0f, 1.5f, 0.2f, 0.5f, 1.8f);
            }
            else if (fieldsOfStudy == Helpers.FieldsOfStudy.Informatics)
            {
                multipliers = CreateMulipliers(0f, 1.0f, 1.5f, 0.7f, 1.3f, 0.5f);
            }

            return multipliers;
        }
        
        private static Dictionary<string, float> CreateMulipliers(float polishBasic, float mathBasic, float mathExtended, float foreinLanguageBasic,
